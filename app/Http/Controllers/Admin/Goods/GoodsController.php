...
class GoodsController extends Controller
{
    ...
    {
        if (empty($this->params['type'])) return ['errcode' => 1, 'errmsg' => '类型必须填写'];
        //排序
        ...
        $query = Goods::where('goods.merchant_id', $this->user_id)->where('goods.is_delete', 1)->orderBy('goods.' . $column, $direction);
        if ($this->params['type'] == 1) {//普通商品
            $query->whereIn('goods.is_sku', [0, 1]);
        } elseif ($this->params['type'] == 2) {//预约商品
        ...
        //判断是在售还是仓库中
        $on_sale = isset($this->params['sale']) ? intval($this->params['sale']) : 1;
        //在售商品
        if ($on_sale == 1) {
        ...
        $prop_store_id = Prop::where('merchant_id', $this->user_id)->where('is_delete', 1)->where('prop_value_type', 1)->value('id');
        if (!empty($this->params['store_id'])) {
            $query->leftJoin('goods_prop', 'goods.id', '=', 'goods_prop.goods_id')
                ->where('goods_prop.merchant_id', $this->user_id)
...
//        $data['data'] = $this->goods_model->get_data_list($wheres, $this->fields, $offset, $limit, $order);
        ...
        foreach ($data['data'] as $key => $item) {
            ...
            if ($item['is_sku'] == 0) {
                ...
                $stock_goodsspec = 0;
                $rs_goodsspec = GoodsSpec::get_data_by_goods_id($item['id'], Auth::user()->merchant_id);
                //dd($rs_goodsspec);
...
//        $goods_res = $this->goods_model->updateByWhere($wheres,$update_data);
        foreach ($ids as $id) {
            $tem_res = Goods::get_data_by_id($id, $this->user_id);
            if ($tem_res->is_sku == 2) {
                //是否有预约商品的权限
                $appointservice_priv = UserPrivService::getUserPrivs();
                if (!in_array('trade_apptmanage', $appointservice_priv)) {
                    return ['errcode' => 1, 'errmsg' => '无操作预约商品权限'];
                }
            }

            $goods_res = Goods::delete_data($id, $this->user_id);
            if ($goods_res === false) {
                break;
            }
        }
    ...
    public function detail()
    {
        ...
        if ($goods_data) {
            ...
            if ($data['data']['is_sku'] == 2) {
                $data['data']['goods_appt'] = GoodsAppt::where('merchant_id', $this->user_id)->where('goods_id', $id)->first();
            }
            $cat_model = new GoodsCat();
...
//            $data['data']['cat_name'] = json_decode($data['data']['collect_fields'],true);
            ...
            //虚拟商品
            $rs_goods_virtual = GoodsVirtual::get_data_by_id($data['data']['id'],$this->user_id);
...
//        if($column == 'stock'){
        $return_res = Goods::update_data($id, $this->user_id, $update_data);
//        }else{
...
//        $return_res = $this->goods_model->updateByWhere($wheres,$update_data);
        $flag = false;
        foreach ($ids as $id) {
//            Goods::forgetCache($id,$this->user_id);
            if ($update_data['onsale'] == 1) {//上架
                //第一版预约商品上架检测
                $goods_res = Goods::get_data_by_id($id, $this->user_id);
                if ($goods_res->is_sku == 2) {
                    $goods_appt_res = GoodsAppt::get_data_by_goods_id($id, $this->user_id);
        ...
        if ($flag === true) {
            if (count($ids) == 1) {
                return ['errcode' => 1, 'errmsg' => '商品上架失败，请更新预约商品信息再上架'];
            } else {
                return ['errcode' => 1, 'errmsg' => '部分商品上架失败，请更新预约商品信息再上架'];
            }
        }
        if ($return_res === false) {
            $res['errcode'] = 1;
    ...
    public function setGoodsGroup()
    {
        ...
        foreach ($ids as $k => $val) {
            foreach ($tag_ids as $key => $value) {
                $insert_data[$i]['merchant_id'] = $this->user_id;
        ...
        foreach ($ids as $k => $val) {
            Goods::forgetCache($val, $this->user_id);
        }
        if (!$res) {
            $data['errcode'] = 1;
            $data['errmsg'] = '操作失败';
    ...
    public function setArea()
    {
        ...
        $insert_data = [];
        foreach ($ids as $k => $val) {
            $goodsInfo = Goods::get_data_by_id($val, $this->user_id);
            if ($this->isAppointmentGoods($goodsInfo['goods_cat_id'])) {
                continue;
            }
            $insert_data[$k]['merchant_id'] = $this->user_id;
...
//        );
        ...
        foreach ($ids as $item) {
            $return_res = Goods::update_data($item, $this->user_id, $update_data);
//            Goods::forgetCache($item,$this->user_id);
            if ($return_res === false) {
                break;
            }
        }
//        $return_res = $this->goods_model->updateByWhere($wheres,$update_data);
    ...
    {
        $data = $this->params;
        $goods_data = $this->parseGoodsData($this->user_id, $data);
        if ($goods_data['errcode'] != 0) {
            return Response::json($goods_data);
        }
        $goods_cat = new GoodsCat();
        if (!$goods_cat->has_appoint_priv($data['goods_cat_id'])) {
            $return_data = array(
                'errcode' => 99001,
                'errmsg' => '没有预约服务权限'
            );
        ...
        if ($goods_id > 0) {
            ...
            if (isset($data['props']) && (!empty($data['props']))) {
                $attrs = $this->parseAddAttr($this->user_id, $goods_id, $data['props']);
            ...
            if ($display_type == 1) {
                //商品图片添加
                $imgs = $this->parseSimpleData($this->user_id, $goods_id, $data['imgs'], 'img');
            } else { //视频----商品图片添加
                $imgs = $this->parseSimpleData($this->user_id, $goods_id, $data['imgs_videos'], 'img');
            }
            ...
            if (isset($data['tags']) && (!empty($data['tags']))) {
                $tags = $this->parseSimpleData($this->user_id, $goods_id, $data['tags'], 'tag_id');
            ...
            if (isset($data['label_id']) && (!empty($data['label_id']))) {
                $labels = $this->parseSimpleData($this->user_id, $goods_id, $data['label_id'], 'label_id');
            ...
            if (isset($data['region_ids']) && (!empty($data['region_ids']))) {
                ...
                $region_data['merchant_id'] = $this->user_id;
            ...
            if (isset($data['content']) && (!empty($data['content']))) {
                $component_data['merchant_id'] = $this->user_id;
            ...
            if (isset($data['specs_val']) && (!empty($data['specs_val']))) {
                foreach ($data['specs_val'] as $key => $item) {
                    $specs[$key]['merchant_id'] = $this->user_id;
...
//                        $specs[$key]['props_str'] = implode(';', explode(':', $item['props_str']));
            ...
            if (isset($data['specs']) && (!empty($data['specs']))) {
                $attrs = $this->parseAddAttr($this->user_id, $goods_id, $data['specs']);
                $rs = \DB::table('goods_prop')->insert($attrs);
                if (!$rs) {
                    $return_data = array(
                        'errcode' => 1,
        ...
        if (isset($data['csv_goods_id']) && (!empty($data['csv_goods_id']))) {
            GoodsCsv::delete_data($data['csv_goods_id'], $this->user_id);
        }
    ...
    public function edit(GoodsRequest $request)
    {
        ...
        $id = $data['id'];
        if (intval($id <= 0)) {
            return Response::json(array('errcode' => 99001, 'errmsg' => '参数错误,商品id不能为空'));
        }
        //修改商品主表
        $goods_data = $this->parseGoodsData($this->user_id, $data);
        if ($goods_data['errcode'] != 0) {
        ...
        $res = Goods::update_data($id, $this->user_id, $goods_data['goods_data']);
        if ($res === false) {
            $return_data = array(
                'errcode' => 1,
                'errmsg' => '商品主表修改失败'
        ...
        if ( $this->getGoodscatCattype($data['goods_cat_id'])==2 ) {
                ...
                $rs = GoodsVirtual::update_data($id, $this->user_id, $data_goodsvirtual);
                if (empty($rs)) {
                    $return_data = array(
                        'errcode' => 1,
                        'errmsg' => '虚拟商品添加失败'
        ...
        elseif ($this->isAppointmentGoods($request->get('goods_cat_id'))) {
            ...
            if (GoodsAppt::where('goods_id', $id)->where('merchant_id', $this->user_id)->count()) {
                $rs = GoodsAppt::update_data($id, $this->user_id, $update_data);
            } else {
                $update_data['goods_id'] = $id;
                $update_data['merchant_id'] = $this->user_id;
        ...
        if (!empty($imgs)) {
            ...
            foreach ($imgs as $key => $item) {
                $img_data[$key]['merchant_id'] = $this->user_id;
        ...
        GoodsImg::forgetCache($id, $this->user_id);
        ...
        if (isset($data['tags']) && !empty($data['tags'])) {
            ...
            foreach ($tag_rs as $key => $item) {
                $tag_data[$key]['merchant_id'] = $this->user_id;
        ...
        if (isset($data['label_id']) && !empty($data['label_id'])) {
            ...
            foreach ($labels as $key => $item) {
                $label_data[$key]['merchant_id'] = $this->user_id;
        ...
        //不配送区域
        if (isset($data['region_ids']) && !empty($data['region_ids'])) {
            $region_update['merchant_id'] = $this->user_id;
        ...
        //商品详情
        if (isset($data['content']) && !empty($data['content'])) {
            $component_update['merchant_id'] = $this->user_id;
        ...
        if (isset($data['props']) && !empty($data['props']) && is_array($data['props'])) {
            $rs = $this->dealGoodsProp($this->user_id, $id, $data['props']);
            if ($rs['errcode'] != 0) {
        ...
        if ($data['is_sku'] == 1 || $data['is_sku'] == 2) {
            if (isset($data['specs']) && !empty($data['specs']) && is_array($data['specs'])) {
                $rs = $this->dealGoodsProp($this->user_id, $id, $data['specs']);
                if ($rs['errcode'] != 0) {
                    return Response::json($rs);
                }
            }
        ...
        GoodsProp::forgetCache($id, $this->user_id);
        ...
        if ($data['is_sku'] == 1 || $data['is_sku'] == 2) {
            if (isset($data['specs_val']) && !empty($data['specs_val'])) {
                ...
                foreach ($specs as $key => $item) {
                    $specs_data[$key]['merchant_id'] = $this->user_id;
...
//                    $specs_data[$key]['props_str'] = implode(';', explode(':', $item['props_str']));
        ...
        GoodsSpec::forgetCacheByGoods($id, $this->user_id);
        $return_data = array(
            'errcode' => 0,
            'errmsg' => '修改成功'
        );
    ...
    private function parseGoodsData($user_id, $data)
    {
        ...
        if ($goods_cat_id <= 0) {
            return array('errcode' => 99001, 'errmsg' => '参数错误,商品分类不能为空');
        }
        $return_data['goods_cat_id'] = $goods_cat_id;//商品分类id
        $title = isset($data['title']) ? trim($data['title']) : '';
        if (empty($title)) {
            return array('errcode' => 99001, 'errmsg' => '参数错误,商品标题不能为空');
        }
        ...
        if ($this->getGoodscatCattype($data['goods_cat_id'])==2) {
            ...
            if (!isset($data['goods_type']) || empty($data['goods_type'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,虚拟商品的商品类型不能为空');
            }
            if (!isset($data['time_type'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,请选择虚拟商品的虚拟商品使用时间类型');
            }
            if ( $data['time_type']==1 ) {
                if ( !isset($data['start_time']) || empty($data['start_time']) ) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,虚拟商品有效期的开始日期不能为空');
        ...
        elseif ($this->isAppointmentGoods($data['goods_cat_id'])) {
            ...
            if (!isset($data['specs']) || empty($data['specs'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,预约商品的规格不能为空');
            }
            if (empty($data['mode'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,请选择预约商品模式');
            }
            if (!isset($data['leadtime_hr']) || !isset($data['leadtime_day'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,预约时间限制不能为空');
            } else {
                if (intval($data['leadtime_hr']) < 0 || intval($data['leadtime_day']) < 0) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,预约时间限制不能小于0');
                }
                if (intval($data['leadtime_hr']) > 48) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,最少需提前不可大于48小时');
                }
                if (intval($data['leadtime_day']) > 30) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,最多可提前不可大于30天');
                }
            }
            if (isset($data['specs']) && !empty($data['specs'])) {
                ...
                if (strpos($spec_str, ':') === false && strpos($spec_str, '-') === false) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,预约商品预约时段不能为空');
                }

                if (strpos($spec_str, '周') === false && strpos($spec_str, '假') === false) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,预约商品预约日期不能为空');
                }
            }
        }
        ...
        if ($data['onsale_type'] == 1 && !isset($data['onsale_time'])) {
            return array('errcode' => 99001, 'errmsg' => '参数错误,定时状态时间必选');
        }
        ...
        if (!empty($goods)) {
            if ($goods_id > 0) {
                ...
                if (!empty($diff_ids)) {
                    return array('errcode' => 99001, 'errmsg' => '参数错误,已存在同样商品名称的商品');
                }
            } else {
                return array('errcode' => 99001, 'errmsg' => '参数错误,已存在同样商品名称的商品');
            }
        ...
        if (empty($data['imgs']) && empty($imgs_videos)) {
            return array('errcode' => 99001, 'errmsg' => '参数错误,商品图片不能为空');
        }
        //展示类型，1图片，2视频
...
//        }
        ...
        if ($data['is_sku'] == 0) {
            ...
            if (!isset($data['specs']) || empty($data['specs'])) {
                return array('errcode' => 99001, 'errmsg' => '参数错误,多规格商品规格不能为空');
            }
            $stock = 0;
...
//        $return_data['stock_type'] = $data['stock_type'];
    ...
    {
        $data = MerchantSetting::get_data_by_id($this->user_id);
    ...
    public function setWarning()
    {
        ...
        $res = MerchantSetting::update_data($this->user_id, $update_data);
        if ($res === false) {
            $res_data['errcode'] = 1;
    ...
    private function versionFunction($version, $num = 1)
    {
        ...
        switch ($version) {
            //免费版
            case 1:
                $MerchantSetting = MerchantSetting::get_data_by_id($this->user_id);
    ...
    {
        $_search = $request->all();
        $_search['merchant_id'] = $this->user_id;   //获取当前商铺的id
