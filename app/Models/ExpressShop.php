...
class ExpressShop extends Model
{

    protected $table = 'express_shop';
    protected $guarded = ['id'];
    public $timestamps = false;

    static function cacheKey($key){
        return CacheKey::get_dada_express($key,'express_shop');
    }
    static function clearCache($info){
        Cache::forget(self::cacheKey('id_'.$info['id']));
        Cache::forget(self::cacheKey('store_id_'.$info['store_id']));
    ...
    {
        $info = self::query()->where($key,'=',$value)->first();
        if($info) {
