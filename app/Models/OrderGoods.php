...
class OrderGoods extends Model
{
    ...
    /**
	 * 获取单条数据
	 * @author shangyazhao@dodoca.com
	 */
    static function get_data_by_order($order_id, $merchant_id, $fields = '*')
    {
        if(!$order_id || !is_numeric($order_id))return;
        if(!$merchant_id || !is_numeric($merchant_id))return;
		$data = self::query()->select(\DB::raw($fields))->where(['order_id'=>$order_id,'merchant_id'=>$merchant_id])->first();
        return $data;
    }
     ...
     */
    static function get_data_order_goods($order_id, $goods_id, $merchant_id, $fields = '*')
    {
        if(!$order_id || !is_numeric($order_id))return;
        if(!$merchant_id || !is_numeric($merchant_id))return;
        $data = self::query()->select(\DB::raw($fields))->where(['order_id'=>$order_id,'goods_id'=>$goods_id,'merchant_id'=>$merchant_id])->first();
        return $data;
    }
     ...
     * @author wangshiliang@dodoca.com
     */
    static function increment_data($id, $merchant_id, $key, $val)
    {
        if(!$id || !is_numeric($id))return;
        if(!$merchant_id || !is_numeric($merchant_id))return;
        $data['updated_time'] = date('Y-m-d H:i:s');
        return self::query()->where(['id'=>$id,'merchant_id'=>$merchant_id])->increment($key,$val);
    }
