...
class LiveBalance extends Model
{

    protected $table = 'live_balance';
     ...
     * @return array
     */
    static function get_sum_by_type($type, $merchant_id){
    
        if(!$type || !is_numeric($type))return;
        if(!$merchant_id || !is_numeric($merchant_id))return;
    
        return self::query()->where('merchant_id','=',$merchant_id)
                            ->where('type','=',$type)
                            ->sum('sum');
    }
