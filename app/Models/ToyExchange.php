...
class ToyExchange extends Model
{
    protected $connection = 'applet_cust';

    protected $table = 'toy_exchange';
    ...
    static public function get_data_list($member_id, $merchant_id)
    {
        ...
        $key = CacheKey::get_toy_exchange_list_by_member_id_key($member_id, $merchant_id);
        ...
        if (!$data) {
            ...
            $data = self::query()->where($where)->orderBy('created_time', 'desc')->get($fields);
            if ($data) {
                $key = CacheKey::get_toy_exchange_list_by_member_id_key($member_id, $merchant_id);
    ...
    static public function insert_data($data)
    {
        ...
        $key = CacheKey::get_toy_exchange_list_by_member_id_key($data['member_id'], $data['merchant_id']);
