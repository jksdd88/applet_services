...
class XcxVersionDaily extends Model
{
    /**
     * 与模型关联的数据表。
     *
     * @var string
     */
    protected $table = 'xcx_version_daily';//会员每天统计
    ...
    static function get_data_by_day($day)
    {
    ...
    static function get_data_sum($wheres=array(), $fields = '*')
    {
        $query = self::query();
        foreach($wheres as $where) {
            $query->where($where['column'], $where['operator'], $where['value']);
        }
        $data = $query->select(\DB::raw($fields))->orderBy('day_time', 'desc')->get();
        return json_decode($data,true);
    }
}