...
class LiveCatalog extends Model
{

    protected $table = 'live_catalog';
    protected $guarded = ['id'];
    public $timestamps = false;

    static function cacheKey($key){
        return CacheKey::get_live_cache($key,'live_catalog');
    }
    static function clearCache($info){
        Cache::forget(self::cacheKey('id_'.$info['id']));
        Cache::forget(self::cacheKey('merchant_id_'.$info['id']));
    }
